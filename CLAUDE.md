...
- **Features used from `2025-11-25`**: server-level `instructions`
  ...
  validation → `403` (`src/transports/http.ts`).
...
- **Current stats**: 54 test files, **759 tests**
...
by the non-factory delete in `resources.ts`). Decline/cancel/`confirm=false`
aborts the delete (`deleted: false` + reason). Clients without the capability
— and failed elicitation round-trips — keep the legacy direct-delete
behaviour. Opt-out: `BOOND_MCP_CONFIRM_DELETE=0|false|no|off` (exposed as the
