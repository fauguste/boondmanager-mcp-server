...
| `MCP_HTTP_ALLOWED_ORIGINS` | origines loopback avec le port (loopback) ou disabled (other) | Comma-separated allowlist de valeurs `Origin` (scheme + host + port) ; `Origin` hors liste → `403` (exigence spec MCP 2025-11-25). Requête **sans** `Origin` toujours acceptée (curl, gateways). `*` = opt-out explicite. Dans l'image Docker (`MCP_HTTP_HOST=0.0.0.0`), la validation est désactivée par défaut : la définir explicitement si le serveur est appelé depuis un navigateur. |
